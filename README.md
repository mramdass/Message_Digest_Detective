...
There is an Autopsy Plugin which uses the MD5 hashes from the NSRL RDS. This plugin will only work well if the system has at least 8GB of RAM and a current processor or else Autopsy will fail to run smoothly. The plugin works differently in that it reads chunks of MD5 data into memory in a hashmap for quick access. Since autopsy operates on a file-by-file basis: as files are being processed, it hashes file-by-file and looks to see if it exists in the MD5 list. It is better to pass the local directory of the image directory into mdd.py instead of using the plugin.
...
You must have a VirusTotal API Key. Use the following link to obtain an API key and enter it in the configure.json file in place of 'None'.  <https://www.virustotal.com/>
